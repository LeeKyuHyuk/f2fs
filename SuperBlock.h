...
typedef struct {
  ...
  uint8_t extension_list[F2FS_MAX_EXTENSION]
                        [F2FS_EXTENSION_LEN]; /* extension array */
  uint32_t cp_payload;
  ...
  f2fs_device devs[MAX_DEVICES];     /* device list */
  ...
  uint32_t crc;                      /* checksum of superblock */
} f2fs_super_block;
